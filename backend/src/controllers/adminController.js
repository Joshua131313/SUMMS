...
export const getRentalAnalytics = async (req, res) => {
    try {
        ...
        if (from || to) {
            ...
            if (from)
                whereClause.bookingDate.gte = new Date(String(from));
            if (to)
                whereClause.bookingDate.lte = new Date(String(to));
        }
        if (city) {
...
export const getGatewayAnalytics = async (req, res) => {
    try {
        ...
        if (from || to) {
            ...
            if (from)
                whereClause.timeStamp.gte = new Date(String(from));
            if (to)
                whereClause.timeStamp.lte = new Date(String(to));
        }
        if (serviceType) {
